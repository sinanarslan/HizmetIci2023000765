...
namespace MebFinalApp.Business.ValidationRules
{
    public class UserLoginViewModelValidator:AbstractValidator<UserLoginViewModel>
    {
        ...
        public UserLoginViewModelValidator(IUserBs _userBs)
        {
            ...
            RuleFor(x => x.UserName).NotEmpty().WithMessage("Lütfen Boş Bırakmayınız");
            /*.MaximumLength(10).WithMessage("Lütfen 10 karakterden az giriniz");*/

            RuleFor(x => x.Password).Must(KullaniciAdiKontrol).WithMessage("Bu Kullanıcı Adı Kullanılıyor");
